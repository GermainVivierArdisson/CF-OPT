...
from src.plot.plot_explanation import plot_explanation
from src.prediction.prediction_models import PartialResNet, LinearRegression


def get_counterfactual(pred_model, dataset, initial_index,
                       alternative_index, vae=None, grid=(12,12), explanation_type="relative",
                       ...
                       plot_result=True, plot_style = "normal", write_tb=False,
    ...
    Args:
      ...
      dataset (PyTorch dataset): dataset from which the initial context and alternative path will be taken from.
      ...
      plot_style (bool): indicates plotting style used.
                         Can be either 'normal' or 'superposed'.
    ...
    if isinstance(pred_model, PartialResNet):
        task = "warcraft"
    elif isinstance(pred_model, LinearRegression):
        task = "grid"
    ...
    if task=="warcraft": grid = (12, 12)

    with gp.Env(empty=True) as env:
        env.setParam('OutputFlag', 0)
        env.start()

        with ShortestPathModel(grid, env=env, task=task) as short_path_model:
            ...
            with torch.no_grad():
                w_init, _ = solve(pred_model(x_init.unsqueeze(0)).squeeze(), short_path_model, task)
            ...
            if explanation_type in ["relative", "absolute"]:
                x_alt, _, w_alt, _ = dataset[alternative_index]
                x_alt = x_alt.to("cpu")
                w_alt = w_alt.to("cpu")
            ...
            for n_steps in range(max_iter):
                ...
                if explanation_type == "relative":
                    ...
                    opt_crit = absolute_optimality_criterion(new_cp.squeeze(), w_alt, short_path_model, task)
                elif explanation_type == "epsilon":
                    opt_crit = epsilon_optimality_criterion(new_cp.squeeze(), w_init, epsilon, short_path_model, task)
            ...
            if plot_result:
                if explanation_type == "epsilon":
                    if best_sol is not None:
                        w_alt, _ = solve(pred_model(best_sol).squeeze(), short_path_model, task)
                    else:
                        w_alt = w_init

                plot_explanation(objective, distance_from_x, best_sol, best_index,
                                first_feasible, first_feasible_index,
                                explanation_type, optimalities, x_init, pred_model,
                                w_init, w_alt, new_x, lambdas, task, plot_style, use_vae)
